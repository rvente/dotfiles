  ...
  LinkHintsMode = (function() {
    ...
    LinkHintsMode.prototype.onKeyDownInMode = function(event) {
      ...
      if ((ref = event.key, indexOf.call(modifiers, ref) >= 0) && ((ref1 = this.mode) === OPEN_IN_CURRENT_TAB || ref1 === OPEN_WITH_QUEUE || ref1 === OPEN_IN_NEW_BG_TAB || ref1 === OPEN_IN_NEW_FG_TAB)) {
        ...
        return this.hintMode.onExit(function() {
          return handlerStack.remove(handlerId);
        });
      } else if (KeyboardUtils.isBackspace(event)) {
        if (this.markerMatcher.popKeyChar()) {
          return this.updateVisibleMarkers();
        } else {
          return this.hintMode.exit(event);
        }
      } else if (event.key === "Enter") {
        if (this.markerMatcher.activeHintMarker) {
          return HintCoordinator.sendMessage("activateActiveHintMarker");
        }
      } else if (event.key === "Tab") {
        this.tabCount = previousTabCount + (event.shiftKey ? -1 : 1);
        return this.updateVisibleMarkers(this.tabCount);
      } else if (event.key === " " && this.markerMatcher.shouldRotateHints(event)) {
        this.tabCount = previousTabCount;
        return HintCoordinator.sendMessage("rotateHints");
      } else {
